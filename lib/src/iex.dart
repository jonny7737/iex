...
class IEX extends BaseAPI {
  ...
  Future<JSONObject> nextMarketOpen() async {
    ...
    JSONObject jsonObject = await this
        .getRequest(function: 'ref-data', market: 'us', types: 'trade', period: 'next', range: '1');
    return jsonObject;
  }

  Future<JSONObject> getSymbolList() async {
    JSONObject jsonObject = await this.getRequest(function: 'ref-data', symbol: 'symbols');
    return jsonObject;
  }

  Future<JSONObject> currentPrice({String symbol}) async {
    JSONObject jsonObject = await this.getRequest(function: 'price', symbol: symbol);
    // await this.getRequest(function: 'latestPrice', symbol: symbol);
    // print('ts currentPrice: ${jsonObject.jsonContents}');
    return jsonObject;
  }

  Future<JSONObject> latestPrice({String symbol}) async {
    JSONObject jsonObject = await this.getRequest(function: 'latestPrice', symbol: symbol);
    // print('ts currentPrice: ${jsonObject.jsonContents}');
    return jsonObject;
  }

  Future<JSONObject> intraDay({String symbol}) async {
    JSONObject jsonObject = await this.getRequest(function: 'intraday-prices', symbol: symbol);
  ...
  Future<JSONObject> stockBatch(
    ...
    if (this.getAPIKey() == null) return JSONObject('{"ERROR": "API key not set."}');
  ...
  Future<JSONObject> ti({String symbol, String ti, String range, String period}) async {
    if (this.getAPIKey() == null) return JSONObject('{"ERROR": "API key not set."}');
    JSONObject jsonObject = await this.getRequest(
      function: 'stock',
      symbol: symbol,
